...
| **One-Click selection editing** | Correct a proposal before Create: hover a space to raise handles, drag a corner (snaps to linework), drag an edge to move the whole line, Shift-click an edge to insert an anchor, select a corner + Delete to remove just that point — SF recomputes live | `TakeoffCanvas.jsx` (`oneClickHandleAt`, `ocDragMove`, `ocHoverUpdate`, `deleteSelectedOcVertex`, `ocMetrics`/`ocSnap`; handle render in the proposal preview) |
| **Selected-shape editing (same model)** | The same handles on a committed takeoff (Select tool): corner drag (snaps), select-corner + Delete removes just that point, edge drag moves both endpoints, Shift-click an edge inserts | `TakeoffCanvas.jsx` (`selectAt` corner/edge arming, `"edge"` drag branch in `onPointerMove`, `deleteSelectedShapeVertex`, `selVert` state; handle render in the selected-shape overlay) |
...
| **Conditions** | One finish each: color + CAD hatch, waste %, ×N multiplier, wall height, border thickness | Takeoffs panel (docked, resizable) in `web/src/components/TakeoffsPanel.jsx`; `HatchPattern` in `web/src/components/hatches.jsx`; totals math in `web/src/lib/totals.js` |
| **Takeoffs panel at scale** | Filter, natural sort, tag-family grouping (all view-only — hotkeys stay positional), ⌘/⇧ multi-select bulk edit, ⌖ zoom-to-condition, optional compact strip | panel block + `renderCondRow`/`condGroups` in `web/src/components/TakeoffsPanel.jsx`; prefs under `opentakeoff_panel` (localStorage) |
| **Condition library** | Browser-wide reusable templates (appearance, waste, H/T, materials); fresh workspaces seed from it | Library tab in `web/src/components/TakeoffsPanel.jsx`; `loadTemplates`/`saveTemplates` in `web/src/lib/store.js` (meta store key) |
| **Material library** | Browser-wide reusable materials; copy-on-attach with a live link (⛓), amber per-field overrides with revert, explicit update-linked propagation, delete detaches | Materials tab + `MaterialsEditor` library props in `web/src/components/TakeoffsPanel.jsx`; `loadMaterialLibrary`/`saveMaterialLibrary` in `web/src/lib/store.js` |
| **Assemblies** | Per-condition supporting materials: coverage rate + basis → order qty rounded up; trowel picker for adhesives | `MaterialsEditor` in `web/src/components/TakeoffsPanel.jsx` (inline in the Takeoffs panel rows); math in `web/src/lib/totals.js` |
...
| **Export** | CSV / **Excel (.xlsx)** — four-tab workbook (Conditions / By sheet / Materials / Shapes), SpreadsheetML hand-rolled in-browser and zipped with the bundled fflate / JSON / print / **Marked Set PDF** — marked sheets with the takeoff burned in (colors, clipped hatch, quantity chips, markups) + a legend cover with net totals and a by-sheet breakdown; follows the current light/dark view | export handlers in `ReportPanel.jsx`; `web/src/lib/xlsx.js`; `web/src/lib/markedset.js` |
...
| **Dark view (negative print)** | ☾ in the zoom cluster inverts sheet pixels in place (one difference pass, an involution — no filter layers, no re-render) with dark hatch/fill variants baked into the patterns | `invertCanvasPixels` in `web/src/lib/canvasUtil.js`; dark pattern ids in `TakeoffCanvas.jsx` |
