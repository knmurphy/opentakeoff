...
| **45°/90° angle guides** | In-progress segment locks to the 45° family (4° tolerance, ⇧ = hard lock); dashed sheet-wide guide; the click commits the on-axis point | `angleSnap()` + the lock block in `moveCrosshair`, `TakeoffCanvas.jsx` |
| **Magnifier crosshair** | Liquid-glass loupe rides the aim point: the sheet blits under the lens at 2.25× current zoom, live angle + segment-length chip | loupe block in `moveCrosshair` + loupe JSX, `TakeoffCanvas.jsx` (`LOUPE_D`, `LOUPE_MAG`) |
