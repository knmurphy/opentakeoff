...
provider is YAGNI until one is real. The point is the cut-line: delete `web/src/lib/
sync/*` and `web/src/lib/google/snapshotSync*` and a fully working local-first app
remains with no dangling imports. The upstream/parent repo can cherry-pick the
snapshot feature (`putSnapshot` + the snapshot-sync module) with zero Drive coupling.
