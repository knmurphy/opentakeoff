...
// ── churn proxy ─────────────────────────────────────────────────────────────
const FRAMES = 20;
const out: any = { probe: "A8b", mask: `${IMG}x${IMG}`, frames: FRAMES, byDoors: [] };
for (const doors of [0, 1, 6]) {
  (globalThis as any).gc?.();
  const { mo, seed } = doorScene(doors);
  hover(mo, seed);                                   // cold, excluded
  (globalThis as any).gc?.();
  let prev = process.memoryUsage().external;
  let churn = 0, drops = 0, hi = prev;
  const t0 = performance.now();
  for (let k = 0; k < FRAMES; k++) {
    hover(mo, seed);
    const e = process.memoryUsage().external;
    if (e > prev) churn += e - prev; else if (e < prev) drops++;
    if (e > hi) hi = e;
    prev = e;
  }
  const ms = performance.now() - t0;
  out.byDoors.push({
    doors, framesMs: +ms.toFixed(1), msPerFrame: +(ms / FRAMES).toFixed(1),
    externalChurnMB: +(churn / 1048576).toFixed(1),
    churnPerFrameMB: +(churn / FRAMES / 1048576).toFixed(1),
    collections: drops,
  });
}
