...
| **Item F precursor** — label precision + seed placement | Matching a 2–3 digit numeral is not enough to call something a room. `roomLabelSeeds` now rejects the plan's own printed areas (`557 SF` — the numeric token survives tokenizing), dimension strings (`58'-5"`), drawing numbers (`08 - 6231`) and title-block sentences (`RENOVATE BUILDING 28`), plus an opt-in drawing-extent gate for sheet-margin numerals (the caller supplies the extent; only it knows where the paper begins). On the VA plan: 56 candidate numerals → 41 seeds, and the largest "room" on the sheet — 847 SF of title block — stops being proposed. Separately, the seed now drops 1.5 text-heights BELOW the label rather than sitting on its baseline: a rectangle drawn around the room tag is a common convention and the anchor sits inside it, so the flood measured the tag (~3.5 SF) instead of the room — 37 of 41 proposals. Below, not beside: the box is ~2.6× the width of the text run. Measured: median proposal 3 → 52 SF, sub-4-SF 37 → 11, tag-reachable rooms 0/2 → 2/2. `PositionedTextItem` gains an optional `h` (device-space glyph height); callers without metrics keep the anchor | `web/src/lib/detectRooms.ts`, `web/test/detectRooms.test.ts` |
