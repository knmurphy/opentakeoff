...
- **Bench: seed-pair stability, scorer mutation tests, human-SF reference rows.** The VA plan's T1-corridor seed instability is now pinned: a gating `t1-corridor` regression golden plus a `seedPairs` known-fail asserting the raw disagreement (two clicks ~8 ft apart: 158.1 vs 1525.8 SF, 9.65×; xpass fires the day the seeds agree within 1.5×, whichever region wins). The scorer itself is now mutation-tested — `polyIoU` against exact analytic rectangle IoU (equality on integer-aligned cases), `ringAreaAbs` against an independently coded shoelace, and a perturbed-vertex probe whose IoU drop must match the analytic value. And the hand-measured campaign's CE-4/CE-5 SF numbers ride as `humanSfProbes` rows through the new `humanSfGate` — honestly labeled **convention-unmatched** (hand wall-to-wall vs engine centerline ring, a 3–11% gap wider than the 2.5% band), so both rows are known-fail and the gate is xpass-only until a centerline-measured human row lands. `pin-goldens.mts` carries the hand-authored `seedPairs`/`humanSfProbes` fields through re-pins untouched.
