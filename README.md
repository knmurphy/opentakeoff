...
**And find any sheet by what's written on it.** A 200-sheet plan set is a haystack; the gallery's search box searches the *text of every sheet at once* — `CPT-1`, `corridor`, room `139A` — and filters to the sheets that carry it, matched terms shown on each card. It reads the PDF's own text layer, so it costs no upload and no OCR (scanned sheets have no text layer and aren't searchable yet — the measured case for and against client-side OCR is in [`docs/CLIENT_SIDE_OCR_RESEARCH.md`](docs/CLIENT_SIDE_OCR_RESEARCH.md)).
