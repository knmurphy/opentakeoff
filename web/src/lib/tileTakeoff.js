...
function summarizeShape(tile_setup, ring_ft, holes_ft) {
  const { classified } = solveTileLayout({ tile_setup, ring_ft, holes_ft });
  ...
  const order = orderTiles({ safeCount: counts.safe, sku: primarySku(tile_setup) });
...
// multiplied). byCond aggregates them (sum of full/cut/corner/hole/safe/
// keptArea_sf, unioned+re-consolidated cutsheet, unioned warnings) and is
// where the condition's own multiplier is later applied — at the report
// seam (tileReportRows), exactly like computeRollTakeoff's orderFt/rollCount:
  ...
  for (const s of shapes || []) {
    ...
    if (!agg) {
      agg = {
        ...
        grout: { bags: 0 },
        order: { figured: 0, withMargin: 0, boxes: 0 },
        cutRows: [],
        warnings: new Set(),
        shapeIds: [],
      };
    ...
    agg.grout.bags += summary.grout.bags;
    agg.order.figured += summary.order.figured;
    agg.order.withMargin += summary.order.withMargin;
    agg.order.boxes += summary.order.boxes;
