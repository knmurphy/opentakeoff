...
export const oneClickOutput = {
  ...
  tier: z.string().optional().describe("Flood receipt (#175): which path produced the region — strict | strict_uncertain | moderate | predominant_soft | trapped. A triage signal, never a gate; open string (treat unknown values as opaque)"),
  soft_frac: z.number().optional().describe("Hatch-bounded fraction of the accepted region's boundary, 0..1"),
  growth_ratio: z.number().optional().describe("Escalated area ÷ strict area; present only for the strict/moderate tiers"),
...
const detectedRoom = z.object({
  ...
  nverts: z.number().int().describe("Vertex count of the traced polygon"),
  hatch_filtered: z.literal(true).optional().describe("Present when hatch/pattern linework was classified out of the boundary"),
  tier: z.string().optional().describe("Flood receipt (#175): which path produced the region (open string; treat unknown values as opaque)"),
  soft_frac: z.number().optional().describe("Hatch-bounded fraction of the accepted region's boundary, 0..1"),
  growth_ratio: z.number().optional().describe("Escalated area ÷ strict area; present only for the strict/moderate tiers"),
  verts: z.array(point).optional().describe("Traced polygon vertices (image px), when return_verts was set"),
  area_sf: z.number().optional().describe("Scaled mode: traced area in SF"),
