...
import {
  extractVectorGeometry, buildMask, traceRegion, oneClickRing, ringArea,
...
import { roomLabelSeeds, seedLadderPx, isLabelBubblePx, floodSurroundsLabelPx, sheetBounds, floodAtSeed, type LabelBBox } from "../../web/src/lib/detectRooms.ts";
...
export interface ShapeOrigin {
  ...
  layer_bounded?: true;
  raster_traced?: true;
  /** The sheet had no scale when this was measured, so the engine ran with its
   * scale-blind fallbacks (hairline seal radii only, no door-swing retry,
   * minimum-passage rule off) — a different measurement from the scaled one. */
  scale_blind?: true;
...
interface FloodEvidence {
  ...
  gapBridged?: number;
  ringWedges?: number;
  /** The sheet had no scale, so the flood ran with the documented scale-blind
   *  fallbacks (hairline seal radii only, no door-swing retry, minimum-passage
   *  rule off). A DIFFERENT measurement from the scaled one, so it is
   *  disclosed on the record rather than left silent. */
  scaleBlind?: true;
}
...
export class Session {
  ...
  private static floodEvidence(f: Extract<FloodResult, { status: "ok" }>, raster: boolean, mppf: number): FloodEvidence {
    return {
      ...
      ...(mppf > 0 ? {} : { scaleBlind: true as const }),
    };
  }
  ...
  private static floodStamp(ev: FloodEvidence, areaSF?: number): Partial<ShapeOrigin> {
    ...
    return {
      ...
      ...(ev.scaleBlind ? { scale_blind: true as const } : {}),
    };
  }
  ...
  async oneClick(name: string, x: number, y: number, opts: { condition?: string; role: "floor_area" | "deduct"; returnVerts: boolean; sensitivity?: number; layers?: { include?: string[]; exclude?: string[] } }) {
    ...
    const ring = oneClickRing(f, raster
  ...
  async detectRooms(name: string, opts: { condition?: string; role: "floor_area" | "deduct"; returnVerts: boolean; minAreaSf?: number; sensitivity?: number; layers?: { include?: string[]; exclude?: string[] }; assignFromSchedule?: boolean }) {
    ...
    const walkLadder = (lb: { str: string; bbox: LabelBBox }): LadderResult => {
      ...
      for (const probe of seedLadderPx(lb.bbox, "below-box")) {
        ...
        if (probe[0] < bounds.x0 || probe[0] > bounds.x1 || probe[1] < bounds.y0 || probe[1] > bounds.y1) continue;
        ...
        // Guard on the PRE-SNAP trace: vertex snap can pull a tag-box ring onto
        // nearby wall endpoints and inflate its bbox just past BUBBLE_RATIO,
        // smuggling the box through the bubble guard. (On the raster path there
        // is no snap, so raw IS the ring — traced once, at the looser eps.)
        const raw = (raster ? traceRegion(f, RASTER_RDP_EPS) : traceRegion(f)) as [number, number][];
        if (raw.length < 3) { sawDegenerate = true; continue; }
        if (isLabelBubblePx(raw, lb.bbox)) { sawBubble = true; continue; }
        // A rung can step across a shared wall into the NEIGHBORING room, and
        // committing that under this label (or ring-key-deduping it into the
        // neighbor's own detection as a false `merged_labels`) silently
        // unmeasures a real room. Ownership is an EITHER test: the label center
        // inside the traced ring (tag box floating free of the walls, or no box
        // at all), OR the flood's region surrounding the label's box
        // (floodSurroundsLabelPx — a box tied to a wall by its leader notches
  ...
  readSheetText(name: string, region?: { x0: number; y0: number; x1: number; y1: number }) {
    ...
    // glyph height, which room detection needs; it is projected out here rather
    // than widening the contract, and the strict conformance check enforces that.
